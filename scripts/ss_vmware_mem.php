...
function ss_vmware_esx_mem($hostname, $host_id, $snmp_auth, $cmd, $arg1 = "", $arg2 = "") {
	...
	if ($cmd == "index") {
		$return_arr = ss_vmware_esx_mem_reindex(cacti_snmp_walk($hostname, $snmp_community, $oids["index"], $snmp_version, $snmpv3_auth_username, 
$snmpv3_auth_password, $snmp_port, $snmp_timeout, read_config_option("snmp_retries"), SNMP_POLLER));
		...
		$arg = $arg1;
		$arr_index = ss_vmware_esx_mem_reindex(cacti_snmp_walk($hostname, $snmp_community, $oids["index"], $snmp_version, $snmpv3_auth_username, 
$snmpv3_auth_password, $snmp_port, $snmp_timeout, read_config_option("snmp_retries"), SNMP_POLLER));
		...
		if ($arg == "index") {
			...
			$arr_vmid = ss_vmware_esx_mem_reindex(cacti_snmp_walk($hostname, $snmp_community, $oids["vmid"], $snmp_version, $snmpv3_auth_username, 
$snmpv3_auth_password, $snmp_port, $snmp_timeout, read_config_option("snmp_retries"), SNMP_POLLER));
			$arr = ss_vmware_esx_mem_vmid_reindex(cacti_snmp_walk($hostname, $snmp_community, $oids[$arg], $snmp_version, $snmpv3_auth_username, 
$snmpv3_auth_password, $snmp_port, $snmp_timeout, read_config_option("snmp_retries"), SNMP_POLLER));
	                for ($i=0;($i<sizeof($arr_index));$i++) {
        	                if ($arr_vmid[$i] == -1) {
                	                print $arr_index[$i] . "!U\n";
			...
			$arr = ss_vmware_esx_mem_reindex(cacti_snmp_walk($hostname, $snmp_community, $oids[$arg], $snmp_version, $snmpv3_auth_username, 
$snmpv3_auth_password, $snmp_port, $snmp_timeout, read_config_option("snmp_retries"), SNMP_POLLER));
			for ($i=0;($i<sizeof($arr_index));$i++) {
